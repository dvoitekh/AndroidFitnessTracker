...
class StepDetector(private var stepCallback: (timeNs: Long) -> Unit) {
    ...
    fun updateAccel(timeNs: Long, x: Float, y: Float, z: Float) {
        ...
        worldZ[0] = accelRingX.sum() / Math.min(accelRingCounter, ACCEL_RING_SIZE)
        worldZ[1] = accelRingY.sum() / Math.min(accelRingCounter, ACCEL_RING_SIZE)
        worldZ[2] = accelRingZ.sum() / Math.min(accelRingCounter, ACCEL_RING_SIZE)
        ...
        val velocityEstimate = velRing.sum()
